...
#if RamdonMode == 0
#define OpenFile fp=fopen("RS6347_EntropyLCC_η=4.txt","a")
#elif RamdonMode == 21
#define OpenFile fp=fopen("RS6347_RPLCC-21_Ω=16.txt","a")
#elif RamdonMode == 22
#define OpenFile fp=fopen("RS6347_RPLCC-22_Ω=16.txt","a")
#elif RamdonMode == 23
#define OpenFile fp=fopen("RS6347_EntropyLCC-23_Ω=16.txt","a")
#endif
...
int dec_codeword2[n];	//decoded codeword for entropy way
float pi = 3.141593;
...
int test_vec2[test_vec_num][n];
int test_vec_com[n - eta];	//store first n-eta symbols
double test_set_ordered[2][n]; // [0] stores value, [1] stores index
double entropy_ordered[2][n];
...
int testCount1_entropy[test_vec_num];
#endif
...
#if RamdonMode == 23
int serial_with_most_proba, serial_with_most_proba2;
#endif
//***************************
	...
	for (SNR = start; SNR <= finish; SNR += interval)
	{
		...
		for (j = 1; j <= seq_num; j++)
		{
			...
			*/
			...
			printf("Progress=%0.1f, SNR=%2.2f, FrameErrors-1=%2.1d, FER-1=%E, FrameErrors-2=%2.1d, FER-2=%E, addNum=%0.2f, mulNum=%0.2f, total_num=%0.2f\r", progress, SNR, ferror, FER, ferror2, FER2, addNum_count, mulNum_count, totalNum_count);
		...
		else
		{
			...
			FER2 = (double)(ferror2) / (double)(seq_num);
		}

		printf("Progress=%0.1f, SNR=%2.2f, FrameErrors-1=%2.1d, FER-1=%E, FrameErrors-2=%2.1d, FER-2=%E, addNum=%0.2f, mulNum=%0.2f, total_num=%0.2f\n", progress, SNR, ferror, FER, ferror2, FER2, addNum_count, mulNum_count, totalNum_count);

		OpenFile;
		fprintf(fp, "Progress=%0.1f, SNR=%2.2f, FrameErrors-1=%2.1d, FER-1=%E, FrameErrors-2=%2.1d, FER-2=%E, addNum=%0.2f, mulNum=%0.2f, total_num=%0.2f\n", progress, SNR, ferror, FER, ferror2, FER2, addNum_count, mulNum_count, totalNum_count);
		fclose(fp);
...
{	//construct the test_vec[test_vec_num]
	int i, j, temp_index;
	float temp;
	double test_set[2][n]; // [0] stores value, [1] stores index
	double large_vec_proba[choose_num][n]; // largest index is 0, second largest index is 1

	//find the ML and second ML elment for every column of RM
	for (i = 0; i < n; i++)
	{
	...
	//cal the \gamma in every column of RM
	for (i = 0; i < n; i++)
	{
	...
	//ordering codeword symbol by \gamma 
	//key part!!
	for (i = 0; i<n; i++)
	{
	...
	for (i = 0; i<(n - 1); i++)
		for (j = 0; j<(n - (i + 1)); j++)
			...
			{
				double temp = test_set_ordered[0][j];
	...
	//cal the entropy in every column of RM
	for (i = 0; i < n; i++)
	{
		entropy_ordered[0][i] = 0.0;
		entropy_ordered[1][i] = i;
		for (j = 0; j < q; j++)
		{
			double temp_entropy = RM[j][i] * (log(RM[j][i]) / log((double)2));
			entropy_ordered[0][i] += temp_entropy;
		}
		entropy_ordered[0][i] = -entropy_ordered[0][i];
	}

	//ordering codeword symbol by entropy
	//key part!!
	for (i = 0; i<(n - 1); i++)
		for (j = 0; j<(n - (i + 1)); j++)
			if (entropy_ordered[0][j]>entropy_ordered[0][j + 1])
			{
				double temp_entropy = entropy_ordered[0][j];
				entropy_ordered[0][j] = entropy_ordered[0][j + 1];
				entropy_ordered[0][j + 1] = temp_entropy;

				temp_index = entropy_ordered[1][j];
				entropy_ordered[1][j] = entropy_ordered[1][j + 1];
				entropy_ordered[1][j + 1] = temp_index;
			}
	...
	//the num of loop equals to eta for \gamma way
	...
	for (i = 0; i<test_vec_num; i++)
	{
		...
		for (j = n - 1; j>n - eta - 1; j--)
		{
			...
			test_vec[i][j] = large_vec[temp_index][(int)test_set_ordered[1][j]];
	...
	for (i = 0; i < test_vec_num; i++)
		for (j = 0; j < n - eta; j++)
		{
			test_vec2[i][j] = large_vec[0][(int)entropy_ordered[1][j]];
		}

	//calculate the num of diffs between codeword and test_vec2[i]
	testCount1_entropy[0] = 0;
	for (i = 0; i<n - eta; i++)
		if (codeword[(int)entropy_ordered[1][i]] != large_vec[0][(int)entropy_ordered[1][i]])
		{
			testCount1_entropy[0]++;
		}

	for (j = 1; j<test_vec_num; j++)
	{
		testCount1_entropy[j] = testCount1_entropy[0];
	}

	//the num of loop equals to eta for entropy way
	for (i = 0; i<test_vec_num; i++)
	{
		mask = 1;
		for (j = n - 1; j>n - eta - 1; j--)
		{
			...
			test_vec2[i][j] = large_vec[temp_index][(int)entropy_ordered[1][j]];

			if (codeword[(int)entropy_ordered[1][j]] != large_vec[temp_index][(int)entropy_ordered[1][j]])
			{
				testCount1_entropy[i]++;
			}
...
{	//construct the test_vec[test_vec_num]
	...
	//contruction test_vec2
	//construct 2^{n-k} = test_vec_num test_vector2
	for (i = 0; i < test_vec_num; i++)
		for (j = 0; j < n; j++)
			test_vec2[i][j] = large_vec[0][(int)entropy_ordered[1][j]];
	...
	for (i = 0; i < test_vec_num; i++)
	{
		...
		for (j = n - 1; j > n - eta_max - 1; j--)
		{
			...
			test_vec2[i][j] = large_vec[bit_index][(int)entropy_ordered[1][j]];
	...
	//ordering test_vec by the value of the posteriori probablility
	double log_proba2[2][test_vec_num];
	for (i = 0; i < test_vec_num; i++)
	{
		log_proba2[1][i] = i;
		log_proba2[0][i] = 0.0;
		...
		{
			for (int u = 0; u < q; u++)
				if (test_vec2[i][j] == root[u])
				{
					//the porba in test_vec[i] is a ordered element's proba, so the column of RM shall be a ordered serial num
					log_proba2[0][i] = log_proba2[0][i] + log(RM[u][(int)entropy_ordered[1][j]]);
					break;
				}
		}
	}
	...
	for (i = 1; i < test_vec_num; i++)
	{
		double temp = log_proba2[0][i];
		j = i - 1;
		while (log_proba2[0][j] <= temp && j >= 0)
		{
			log_proba2[0][j + 1] = log_proba2[0][j];
			log_proba2[1][j + 1] = log_proba2[1][j];
			j--;
		}
		log_proba2[0][j + 1] = temp;
		log_proba2[1][j + 1] = i;
	}
	serial_with_most_proba2 = (int)log_proba2[1][0];
	...
	//calculate the num of diffs between codeword and test_vec2[i]
	for (i = 0; i < test_vec_num; i++)
		testCount1_entropy[i] = -1;

	for (i = 0; i < choosen_tv_num; i++)
	{
		temp_index = (int)log_proba2[1][i];
		testCount1_entropy[temp_index] = 0;
		for (j = 0; j < n; j++)
			if (codeword[(int)entropy_ordered[1][j]] != test_vec2[temp_index][j])
			{
				testCount1_entropy[temp_index]++;
			}
	}
...
void cheatDecoding(void)
{
	int i, u, v, value, flag_judge, flag_judge2;
	unsigned int mask = 1;
	...
	//start judgment for testCount1_entropy
	flag_judge2 = 0;	//used to judge if there is correct codeword, 0-->No, 1-->Yes
	for (i = 0; i<test_vec_num; i++)
		if (testCount1_entropy[i] != -1 && testCount1_entropy[i] <= errorCorrectionNum)
		{
			flag_judge2 = 1;
			for (u = 0; u<n; u++)
			{
				dec_codeword2[u] = codeword[u];
			}
			break;
		}
	...
	//for test_vec2[] in entropy way
	if (!flag_judge2)	//flag_judge==0 means that there is not correct codeword
	{
		//select the dec_codeword by the posteriori probablility
		double log_proba[test_vec_num];
		double log_proba_temp = 0.0;
		...
		int max_num = -1;
		for (int j = 0; j<test_vec_num; j++)
			if (testCount1_entropy[j] > -1)
			{
				log_proba[j] = 0.0;
				for (i = 0; i<n; i++)
				{
					for (u = 0; u<q; u++)
						if (test_vec2[j][i] == root[u])
						{
							//the porba in test_vec[j] is a ordered element's proba, so the column of RM shall be a ordered serial num
							log_proba[j] = log_proba[j] + log(RM[u][(int)entropy_ordered[1][i]]);
							break;
						}
				}
		...
		if (max_num != -1)
		{
			for (u = 0; u < n; u++)
			{
				dec_codeword2[(int)entropy_ordered[1][u]] = test_vec2[max_num][u];
			}
		}
		else {
			printf("\nThe selecting process in checkDecoding has error!\n");
	...
	if (!flag_judge2)	//flag_judge==0 means that there is not correct codeword
	{
		for (u = 0; u < n; u++)
		{
			dec_codeword2[(int)entropy_ordered[1][u]] = test_vec2[serial_with_most_proba2][u];
		}
	}
#endif
