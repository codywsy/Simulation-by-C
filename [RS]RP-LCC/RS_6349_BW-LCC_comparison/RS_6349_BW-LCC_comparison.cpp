...
#if RamdonMode == 11
#define OpenFile fp=fopen("RS6349_BW-LCC_comparsion.txt","a")
#elif RamdonMode == 12
#define OpenFile fp=fopen("RS6349_BW-LCC-2_comparsion.txt","a")
#endif
...
void main()
{
	...
	for (SNR = start; SNR <= finish; SNR += interval)
	{
		...
		for (j = 1; j <= seq_num; j++)
		{
			...
			if (flag_judge_2 == 0 && flag_judge_1 == 1)
			{
				BW_ferror++;
			}

			if (flag_judge_1 == 0 && flag_judge_2 == 1)
			{
				LCC_ferror++;
			}
			...
			BW_FER = (double)(BW_ferror) / (double)(j);
			LCC_FER = (double)(LCC_ferror) / (double)(j);
			printf("Progress=%0.1f, SNR=%2.2f, Frame Errors LCC=%2.1d, FER_LCC=%E, Frame Errors BW=%2.1d, FER_BW=%E, Only_LCC_FER=%E, OnlyBW_FER=%E\r", progress, SNR, ferror, FER, ferror2, FER2, LCC_FER, BW_FER);
		...
		else
		{
			...
			BW_FER = (double)(BW_ferror) / (double)(seq_num);
			LCC_FER = (double)(LCC_ferror) / (double)(seq_num);
		}

		
		

		printf("Progress=%0.1f, SNR=%2.2f, Frame Errors LCC=%2.1d, FER_LCC=%E, Frame Errors BW=%2.1d, FER_BW=%E, Only_LCC_FER=%E, OnlyBW_FER=%E\n", progress, SNR, ferror, FER, ferror2, FER2, LCC_FER, BW_FER);

		OpenFile;
		fprintf(fp, "Progress=%0.1f, SNR=%2.2f, Frame Errors LCC=%2.1d, FER_LCC=%E, Frame Errors BW=%2.1d, FER_BW=%E, Only_LCC_FER=%E, OnlyBW_FER=%E\n", progress, SNR, ferror, FER, ferror2, FER2, LCC_FER, BW_FER);
		fclose(fp);
...
void cheatDecoding(void)
{
	...
	//start judgment
	flag_judge_1 = 0;	//used to judge if there is correct codeword, 0-->No, 1-->Yes
	for (i = 0; i<test_vec_num; i++)
	...
	if (!flag_judge_1)	//flag_judge==0 means that there is not correct codeword
	{
		for (u = 0; u<n; u++)
		{
...
#if RamdonMode > 0
	flag_judge_2 = 0;	//used to judge if there is correct codeword, 0-->No, 1-->Yes
	for (i = 0; i<tv_num_bit; i++)
	...
	if (!flag_judge_2)	//flag_judge==0 means that there is not correct codeword
	{
		for (u = 0; u<n; u++)
		{
			dec_codeword_bit[u] = large_vec[0][u];
		}
	}
#endif
