...
#define myWay
#define OpenFile fp=fopen("Herm(512,314)_KV_l=1.txt","a")
...
//coefficientSearch()
#define max_m 2	//design large enough to cover, dependent on the value of lm 
#define max_alpha 1	//alpha < max_m, designing alpha large enough to cover all the m_ij of Mulplicity Matrix
#define tableSize_a 950	//more than max_a, can be achieved by the hint of coefficientSearch()
#define tableSize_alpha 989	//more than max_alpha, can be achieved by the hint of coefficientSearch()
//zerobasie()
#define zb_alpha 989	//size+1, larger than tableSize_alpha, according to the maxDeg_x and maxDeg_y of PbMax, we can judge the max num Zb we need
...
//mono_table()
#define weight_Zsize 30
#define weight_XYsize 12466	//equals to the tg_size
#define mono_ordinSize 24940	//choose the value with debugging
#define monoTable_Zsize	(lm+1) 	//equals to the interpoly_Zsize
#define monoTable_Ysize 1382	//large than interpoly_Ysize, but we should avoid to the array crossing the border
#define monoTable_Xsize (w+1)	//large than the interpoly_Xsize
#define monoTable_totalSize 12465 //>index of term[max(deg_y)][w] in the pole basis + 1, equals to tg_size
//*******************
...
#define interpoly_Ysize 1380	//maxdeg of y is (w-1) + w*(iterNum_max/(w+1)), so the Ysize should add 1 more.
...
#define facpoly_Ysize 1380		// same as the interpoly_Ysize
#define facpoly_Xsize (w+1)	// same as the interpoly_Xsize
//rcs()
#define rcspoly_Ysize 1440	//degY+1, degY = interpoly_Ysize + max(j_1) + w*( (max(i_1)+w)/(w+1) ), rcspoly_Ysize = (expoly_Ysize-1) + (facpoly_Ysize-1) + 1
#define rcspoly_Xsize 18	//degX+1, degX = max(i_1) + w, rcspoly_Xsize = (expoly_Xsize-1) + (facpoly_Xsize-1) + 1
...
void mono_table()
{
	...
	//check whether the mono_order_1 is finished 
	for (i = 0; i<monoTable_Zsize; i++)
		for (j = weight_XYsize - 1; j>weight_XYsize - 1 + 100; j--)
			if (mono_order_1[i][j] == -1)
			{
				printf("\nthe mono_order_1 has error!\n");
				break;
			}
