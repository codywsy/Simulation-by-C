...
#define myWay
#define OpenFile fp=fopen("Herm(512,409)_KV_l=1.txt","a")
...
#define max_alpha 4	//alpha < max_m, designing alpha large enough to cover all the m_ij of Mulplicity Matrix
#define tableSize_a 1285	//more than max_a, can be achieved by the hint of coefficientSearch()
#define tableSize_alpha 20	//more than max_alpha, can be achieved by the hint of coefficientSearch()
//zerobasie()
#define zb_alpha 1400	//size+1, larger than tableSize_alpha, according to the maxDeg_x and maxDeg_y of PbMax, we can judge the max num Zb we need
...
//tgorder()
#define tg_size 25083	//more than k, tg_size represent the probably used pole basis num, (w+1)*w/2 + (w+1) * ( (w+interpoly_Ysize)-w+1 ) 
//mono_table()
#define weight_Zsize 78
#define weight_XYsize 25083	//equals to the tg_size
#define mono_ordinSize 26133	//choose the value with debugging
#define monoTable_Zsize	(lm+1) 	//equals to the interpoly_Zsize
#define monoTable_Ysize 2783	//large than interpoly_Ysize, but we should avoid to the array crossing the border
#define monoTable_Xsize (w+1)	//large than the interpoly_Xsize
#define monoTable_totalSize 25083 //>index of term[max(deg_y)][w] in the pole basis + 1, equals to tg_size
//*******************
...
#define interpoly_Ysize 2736	//maxdeg of y is (w-1) + w*(iterNum_max/(w+1)), so the Ysize should add 1 more.
...
#define facpoly_Ysize 2736		// same as the interpoly_Ysize
#define facpoly_Xsize (w+1)	// same as the interpoly_Xsize
//rcs()
#define rcspoly_Ysize 2790	//degY+1, degY = interpoly_Ysize + max(j_1) + w*( (max(i_1)+w)/(w+1) ), rcspoly_Ysize = (expoly_Ysize-1) + (facpoly_Ysize-1) + 1
#define rcspoly_Xsize 25	//degX+1, degX = max(i_1) + w, rcspoly_Xsize = (expoly_Xsize-1) + (facpoly_Xsize-1) + 1
//expoly()-->expanded polynomial
#define expoly_Ysize ((lm*faiMax_Ysize)+1)	
#define expoly_Xsize ((lm*faiMax_Xsize)+1)
//polebasis
#define faiMax_Ysize 49	//change with diff w and k, the max degY of probably used polebasis
#define faiMax_Xsize 8	//change with diff w and k, the max degX of probably used polebasis
...
int expoly[lm+1][expoly_Ysize][expoly_Xsize];	//expanded polynomial in [z+f_k-1-u*pb_k-1-u]^rs, expoly[rs][3>(max(deg_y) in encoding functions)*(rs-1)][3>(max(deg_x) in encoding functions)*(rs-1)]
	...
	unsigned long int coefficientNum, commonNum, facNum, chooseNum;
#endif
...
#ifdef printfAverIterNum
	float count_AverIterNum = 0.0;
#endif
...
#ifdef	_PrintComplexity_
	double commonNum_count, facNum_count, chooseNum_count;
#endif
...
//**the part of coefficient search
	...
	FILE * fp_coeffTable;
	if( (fp_coeffTable=fopen("coeff_table_314.txt","r")) == NULL)
	{
		printf("\nopenfile is failed\n");
	}
	else
...
//			test_vec_contruction();
			...
			chooseNum_count = chooseNum_count + (chooseNum - chooseNum_count) / (double)(j);
#endif

#ifdef printfAverIterNum
		    count_AverIterNum = count_AverIterNum + (iterNum - count_AverIterNum) / (double)(j);
#endif
			...
			printf("Progress=%0.1f, SNR=%2.2f, Bit Errors=%2.1d, BER=%E, Frame Errors=%2.1d, FER=%E, addNum=%0.2f, mulNum=%0.2f, total_num=%0.2f, Choice Errors=%2.1d, CWR=%E, Max_M=%d, Frame_Num=%d\r", progress, SNR, error, BER, ferror, FER,	addNum_count, mulNum_count, totalNum_count, ChosenWrong_SeqNum, CWR, Max_M, j);
		...
		printf("Progress=%0.1f, SNR=%2.2f, Bit Errors=%2.1d, BER=%E, Frame Errors=%2.1d, FER=%E, addNum=%0.2f, mulNum=%0.2f, total_num=%0.2f, Choice Errors=%2.1d, CWR=%E, Max_M=%d, Frame_Num=%d\n", progress, SNR, error, BER, ferror, FER, addNum_count, mulNum_count, totalNum_count, ChosenWrong_SeqNum, CWR, Max_M, j);

		OpenFile;
		fprintf(fp,"Progress=%0.1f, SNR=%2.2f, Bit Errors=%2.1d, BER=%E, Frame Errors=%2.1d, FER=%E, addNum=%0.2f, mulNum=%0.2f, total_num=%0.2f, Choice Errors=%2.1d, CWR=%E, Max_M=%d, Frame_Num=%d\n", progress, SNR, error, BER, ferror, FER, addNum_count, mulNum_count, totalNum_count, ChosenWrong_SeqNum, CWR, Max_M, j);
		fclose(fp);

#ifdef	_PrintComplexity_
		FILE *fp= fopen("3parts Complexity.txt", "a");
		fprintf(fp, "commonNum = %0.2f\tfacNum = %0.2f\tchooseNum = %0.2f\n", commonNum_count, facNum_count, chooseNum_count);
		fclose(fp);
#endif

#ifdef printfAverIterNum
		FILE* fp_iternum=fopen("AverIterNum.txt","a");
		fprintf(fp_iternum,"\nAverIterNum = %0.5f/n", count_AverIterNum);
		fclose(fp_iternum);
#endif
...
//	int q_temp[lm+1][rcspoly_Ysize+w][rcspoly_Xsize];	//q_temp[z-deg+1][y_size>max(deg_y)+1+(rs-1)*(max(deg_y) in encoding functions)][14>w+(rs-1)*w], lc[rs]--leading coefficient polynomial
	...
	//dynamic assignment three dimeension array q_temp[lm+1][rcspoly_Ysize+w][rcspoly_Xsize]
	int *** q_temp = NULL;
	q_temp = Create3DActiveArray(q_temp,(lm+1),(rcspoly_Ysize+w),rcspoly_Xsize);
		...
		for(i=0;i<lm+1;i++)	//2>rs
		{
			...
			{	
				...
				{	
					...
					Free3DActiveArray(q_temp,(lm+1),(rcspoly_Ysize+w),rcspoly_Xsize);
					rcs(r);
...
#ifdef _PrintComplexity_
	chooseNum = (addNum - addNum_temp) + (mulNum - mulNum_temp);
#endif  
...
void tgorder()
{
	...
	*/
	index_temp = 2790;	//according to the last formualtion to calculate out
...
//	int effTable[n][tableSize_alpha][tableSize_a];
	...
	for(i=0;i<n;i++)
	{
		printf("coeff_table p_%d starts to calculate!\n", i);
