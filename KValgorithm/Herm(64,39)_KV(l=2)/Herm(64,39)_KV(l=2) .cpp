...
#ifdef Cost_OR_LM
#define OpenFile fp=fopen("Herm(64,39)_KV_c=75.txt","a")	//define Cost
#define LCC_interpolation_cost 75	//LCC(η=3)'s interpolation cost
#else
#define OpenFile fp=fopen("Herm(64,39)_KV_l=2.txt","a")	//define LM
#endif
...
//tgorder()
#define tg_size 5396	//more than k, tg_size represent the probably used pole basis num, (w+1)*w/2 + (w+1) * ( (w+interpoly_Ysize)-w+1 ) 
//mono_table()
#define weight_Zsize 105
#define weight_XYsize 5395	//equals to the tg_size
#define mono_ordinSize 5667	//choose the value with debugging
#define monoTable_Zsize	(lm+1) 	//equals to the interpoly_Zsize
#define monoTable_Ysize 1080	//large than interpoly_Ysize, but we should avoid to the array crossing the border
#define monoTable_Xsize (w+1)	//large than the interpoly_Xsize
#define monoTable_totalSize 5395 //>index of term[max(deg_y)][w] in the pole basis + 1, equals to tg_size
//*******************
...
#define interpoly_Ysize 1080	//maxdeg of y is (w-1) + w*(iterNum_max/(w+1)), so the Ysize should add 1 more.
...
#define facpoly_Ysize 1080		// same as the interpoly_Ysize
#define facpoly_Xsize (w+1)	// same as the interpoly_Xsize
//rcs()
#define rcspoly_Ysize 1135	//degY+1, degY = interpoly_Ysize + max(j_1) + w*( (max(i_1)+w)/(w+1) ), rcspoly_Ysize = (expoly_Ysize-1) + (facpoly_Ysize-1) + 1
#define rcspoly_Xsize 35	//degX+1, degX = max(i_1) + w, rcspoly_Xsize = (expoly_Xsize-1) + (facpoly_Xsize-1) + 1
//expoly()-->expanded polynomial
#define expoly_Ysize ((lm*faiMax_Ysize)+1)	
#define expoly_Xsize ((lm*faiMax_Xsize)+1)
//polebasis
#define faiMax_Ysize 11	//change with diff w and k, the max degY of probably used polebasis
...
void MatrixConvert( )
{
	...
	//start
#ifdef Cost_OR_LM
	while ((s>0) && (iterNum_temp < LCC_interpolation_cost) && (lM <= lm))
	{
		max_temp = 0.0;
		index_j = -1;
		index_i = -1;
...
void tgorder()
{
	...
	*/
	index_temp = 520;	//according to the last formualtion to calculate out
