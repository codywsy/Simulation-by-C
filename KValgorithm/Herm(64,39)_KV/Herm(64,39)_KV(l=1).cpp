...
#else
#define OpenFile fp=fopen("Herm(64,39)_KV_l=1.txt","a")	//define LM
#endif
...
//rcs()
#define rcspoly_Ysize 175	//degY+1, degY = interpoly_Ysize + max(j_1) + w*( (max(i_1)+w)/(w+1) ), rcspoly_Ysize = (expoly_Ysize-1) + (facpoly_Ysize-1) + 1
#define rcspoly_Xsize 13	//degX+1, degX = max(i_1) + w, rcspoly_Xsize = (expoly_Xsize-1) + (facpoly_Xsize-1) + 1
	...
	for(i=0;i<interpoint_num;i++)
	{
		...
		for(alpha=0;alpha<mulpi;alpha++)
			for(beta=0;beta<(mulpi-alpha);beta++)
			{
				...
				#ifdef checkInter
				//check interpolation in every pair of (alpha,beta)
				int delta_temp;	
