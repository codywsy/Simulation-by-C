...
#define OpenFile fp=fopen("[cheatingAll]LCC_Herm(512,314)_η=3.txt","a")
...
//tgorder()
#define tg_size 4608	//more than k, tg_size represent the probably used pole basis num, (w+1)*w/2 + (w+1) * ( (w+interpoly_Ysize)-w+1 ) 
//mono_table()
#define weight_Zsize 16 
#define weight_XYsize 4608	//equals to the tg_size
#define mono_ordinSize 4975	//choose the value with debugging
#define monoTable_Zsize	(lm+1) 	//equals to the interpoly_Zsize
#define monoTable_Ysize 518	//large than interpoly_Ysize
#define monoTable_Xsize (w+1)	//large than the interpoly_Xsize
#define monoTable_totalSize 4608 //>index of term[max(deg_y)][w] in the pole basis + 1, nearly equals to tg_size
//polebasis()
...
//rcs()
#define rcspoly_Ysize 506	//degY+1, degY = interpoly_Ysize + max(j_1) + w*( (max(i_1)+w)/(w+1) )
#define rcspoly_Xsize 17	//degX+1, degX = max(i_1) + w
#define faiMax_Ysize 37	//change with diff w and k, the max degY of probably used polebasis(polebasis_k-1)
...
//			message[0]=0;	message[1]=0;	message[2]=0;	message[3]=0;
			...
			CWR=(double)(ChosenWrong_SeqNum)/(double)(DecSucc_SeqNum);	//the rate of choosing wrong
...
*/	//*********************
	...
	for(i=0;i<test_vec_num;i++)
	{
		...
		for(j=0;j<init_polyNum;j++)	//find out the leaing order of each Q_j, and deg(Q) = deg(ML)
		{
			temp=-1;
			degree_temp[i][j]=0;
			//*******debug*******
					...
					for(z=0;z<interpoly_Xsize;z++)	//deg_x
						if( Q_uncom_elem[i][j][u][v][z]!=0 )
							if( temp < mono_order[u][v][z] )
							{
								...
								//***debug**************
								degree_test_temp[i][j]=z*w+v*(w+1)+u*weiz;	//w*deg_x + (w+1)*deg_y + weiz*deg_z
								//***********************
							}

			degree_temp[i][j]=temp;	//degree_temp --> lod(Q), degree_test_temp --> degree(Q)
		}

		//choose the Q_min with min degree
		temp_index=0;
...
//	float proba[lm*test_vec_num], proba_temp, temp;
	...
	if(flag==1)	//exist a valid solution
	{
...
//		encoder(outputList[min_index_1][min_index_2],dec_codeword);
	...
	else if(flag==0)	// not exist a valid solution
	{
		for(u=0;u<n;u++)
			dec_codeword[u]=large_vec[0][u];
