...
#include "FiniteFieldBasisCalGF(16).h"
...
//	int degree_temp[test_vec_num][init_polyNum];
	...
	unsigned long int addNum, mulNum;
...
void choose()
{
	int i, j, u, m, v, min_num, value, flag, min_index_1, min_index_2;
	unsigned int mask=1;
	float proba[lm*test_vec_num], temp_x, temp_y, proba_temp, temp;
...
//		encoder(outputList[min_index/lm][min_index%lm],dec_codeword);
