...
#define OpenFile fp=fopen("[cheatingAll]LCC_Herm(512,409)_η=7.txt","a")
//#define OpenFile fp=fopen("[cheatingFac]LCC_Herm(512,409)_η=1.txt","a")
...
//rcs()
#define rcspoly_Ysize 517	//degY+1, degY = interpoly_Ysize + max(j_1) + w*( (max(i_1)+w)/(w+1) )
#define rcspoly_Xsize 20	//degX+1, degX = max(i_1) + w
#define faiMax_Ysize 49	//change with diff w and k, the max degY of probably used polebasis(polebasis_k-1)
