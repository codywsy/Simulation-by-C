...
#define OpenFile fp=fopen("[com_complexity]LCC_Herm(64,49)_eta=3.txt","a")
...
#define able_correct 4	//sakata algorithm decoding capability
#define pointNum 2
#define interval 1
...
#define monoTable_totalSize 275 //>index of term[max(deg_y)][w] in the pole basis + 1, equals to tg_size
//polebasis()


//interpolation()
#define init_polyNum (w*(lm+1))	//change with diff lm, the poly num of the init polyGroup
#define Cominterpoly_Zsize (lm+1)	//maxValue of z is lm=1, so the Zsize should add 1 more.
#define Cominterpoly_Ysize 52	//maxdeg of y is (w-1) + w*(n/(w+1)), so the Ysize should add 1 more.
#define Cominterpoly_Xsize	(w+1)	//maxdeg of x is w, so the Xsize should add 1 more.
...
//			message[0]=0;	message[1]=0;	message[2]=0;	message[3]=0;
			...
			else if(error==temp) //decoding success
			{
				...
				v++;	//the num of decoding successed frame
		...
		printf("Progress=%0.1f, SNR=%2.2f, Bit Errors=%2.1d, BER=%E, Frame Errors=%2.1d, FER=%E, addNum=%0.2f, mulNum=%0.2f, total_num=%0.2f, Choice Errors=%2.1d, CWR=%E, Time=%.2f\n", progress, SNR, error, BER, ferror, FER, addNum_count, mulNum_count, totalNum_count, ChosenWrong_SeqNum, CWR, (double)clock()/CLOCKS_PER_SEC);

		OpenFile;
		fprintf(fp,"Progress=%0.1f, SNR=%2.2f, Bit Errors=%2.1d, BER=%E, Frame Errors=%2.1d, FER=%E, addNum=%0.2f, mulNum=%0.2f, total_num=%0.2f, Choice Errors=%2.1d, CWR=%E, Time=%.2f\n", progress, SNR, error, BER, ferror, FER, addNum_count, mulNum_count, totalNum_count, ChosenWrong_SeqNum, CWR, (double)clock()/CLOCKS_PER_SEC);
		fclose(fp);
				...
				if(J[j]==1)
				{
					...
					else if(j==j_min)
					{
						...
						for(u=0;u<interpoly_Zsize;u++)	//rs
							for(v=0;v<interpoly_Ysize;v++)	//max(deg_y)+1
								for(z=0;z<interpoly_Xsize;z++)	//w+1	
									if( g1[u][v][z]!=0 || g2[u][v][z]!=0 )	//TODO 简便复杂度
									{
	...
	for(j=0;j<init_polyNum;j++)
		if(act[j]==1)
			for(u=0;u<interpoly_Zsize;u++)
			{
				...
				for(v=0;v<interpoly_Ysize;v++)	//max(deg_y)+1
					for(z=0;z<interpoly_Xsize;z++)	//w+1
						if(inGroup_temp[j][u][v][z]!=0) //TODO 简便复杂度
						{
							flag_mulNum = 0;
			...
			if(J[0][j]==1)
			{
				...
				else if(j==j_min[0])
				{
					...
					for(u=0;u<interpoly_Zsize;u++)	//rs
						for(v=0;v<interpoly_Ysize;v++)	//max(deg_y)+1
							for(z=0;z<interpoly_Xsize;z++)	//w+1
								if(inGroup_temp[index_min][u][v][z]!=0) //TODO 简便复杂度
								{
						...
						else if(j>0)
						{
							...
							for(u=0;u<lm+1;u++)	//rs
							{
								for(m=0;m<expoly_Ysize;m++)	//18>(max(deg_y) in encoding functions)*(rs-1)
								{
									...
									{
										if(expoly[u][m][z]!=0)	//TODO 简便复杂度
										{
											for(v=0;v<facpoly_Ysize;v++)	//y_size
											{
												for(t=0;t<facpoly_Xsize;t++)	//w+1
												{
													if(Q[uu][j][v][t]!=0) //TODO 简便复杂度
													{
