...
//the MSB is the first component (indexed by 0) of the binary vector
	...
	for (i = 1; i < m_num_element-1; i++){
		Exp[i] = 2*Exp[i-1];
		if (Exp[i] >= m_num_element){			// if里面的操作是针对超过2^m的元素进行有限域表示？？
	...
	Vec = new int*[m_num_element];	// Vec 为 整数指针数组
	for (i = 0; i < m_num_element; i++)	// 将四个指针分别指向四个数组[2]
		Vec[i] = new int[m_len_element];

	for (i = 0; i < m_num_element; i++)	//求解有限域元素所对应的向量表达形式？
