...
#define monoTable_Ysize 67	//large than Max(interpoly_Ysize,rcspoly_Ysize)
#define monoTable_Xsize (w+1)	//large than interpoly_Xsize
#define monoTable_totalSize 350 //>index of term[max(deg_y)][w] in the pole basis + 1, nearly equals to tg_size
//polebasis()
#define pbNum 275	//key elment, less than tg_size
...
#define interpoly_Ysize 52	//maxdeg of y is (w-1) + w*(iterNum/(w+1)), so the Ysize should add 1 more.
...
#define faiMax_Ysize 10	//change with diff w and k, the max degY of probably used polebasis
...
void main()
{
	...
	FILE *fp;
	if((fp=fopen("LCC_Herm(64,47)eta=1_normal.txt","a"))==NULL)
	{
...
//			message[0]=0;	message[1]=0;	message[2]=0;	message[3]=0;
			...
			printf("Progress=%0.1f, SNR=%2.2f, Bit Errors=%2.1d, BER=%E, Frame Errors=%2.1d, FER=%E, channelError_count=%0.2f, successError_count=%0.2f\r", progress, SNR, error, BER, ferror, FER, channelError_count, successError_count);
		...
		printf("Progress=%0.1f, SNR=%2.2f, Bit Errors=%2.1d, BER=%E, Frame Errors=%2.1d, FER=%E, channelError_count=%0.2f, successError_count=%0.2f\n", progress, SNR, error, BER, ferror, FER, channelError_count, successError_count);

		fp=fopen("LCC_Herm(64,47)eta=1_normal.txt","a");
		fprintf(fp,"Progress=%0.1f, SNR=%2.2f, Bit Errors=%2.1d, BER=%E, Frame Errors=%2.1d, FER=%E, channelError_count=%0.2f, successError_count=%0.2f\n", progress, SNR, error, BER, ferror, FER, channelError_count, successError_count);
		fclose(fp);
					...
					else if(j==j_min)
					{
						...
						for(u=0;u<interpoly_Zsize;u++)	//rs
						{
							for(v=0;v<interpoly_Ysize;v++)	//max(deg_y)+1
							{
								...
								{
									temp=g1[u][v][w+1];
									g1[u][v+1][0]=add(g1[u][v+1][0],temp);
									g1[u][v+w][0]=add(g1[u][v+w][0],temp);
									g1[u][v][w+1]=0;
								}
							}
						}
...
void uncom_elem_interpolation(int interpoint[4],int inGroup[][interpoly_Zsize][interpoly_Ysize][interpoly_Xsize],int outGroup1[][interpoly_Zsize][interpoly_Ysize][interpoly_Xsize],int outGroup2[][interpoly_Zsize][interpoly_Ysize][interpoly_Xsize])
{
	...
	int i, j, u, v, z, J[init_polyNum][2], act[init_polyNum], lod_temp, lod[init_polyNum], lod_min[2], j_min[2];	//(delta, J, act, lod)[num of polys]
	int f[interpoly_Zsize][interpoly_Ysize][interpoly_Xsize];  //g[num_polys][z-deg+1][max(deg_y)+1][w+1], g1[z-deg+1][max(deg_y)+1][w+2], (g2, f)[z-deg+1][max(deg_y)+1][w+1]	
	int g1[interpoly_Zsize][interpoly_Ysize][interpoly_Xsize+1], g2[interpoly_Zsize][interpoly_Ysize][interpoly_Xsize];
	...
	for(i=0;i<2;i++)
	{
		...
		for(j=0;j<init_polyNum;j++)
		{
			...
			{
				J[j][i]=1;	//record those polynomial with a nonzero hasse derivative mapping
				lod_min[i]=lod[j];
				j_min[i]=j;
			}
		}
...
void rcs(int uu)
{
	int i, j, u, v, m, z, t, r, i_1, j_1, i_2, j_2, a, b, leadMono, leadMono_temp, alpha, act, lc[lm+1], q_temp[lm+1][rcspoly_Ysize][rcspoly_Xsize];	//q_temp[z-deg+1][y_size>max(deg_y)+1+(rs-1)*(max(deg_y) in encoding functions)][14>w+(rs-1)*w], lc[rs]--leading coefficient polynomial
	int d1,d2;
...
void choose()
{
	...
	unsigned int mask=1;
	float proba[(lm+1)*test_vec_num+1],temp_x,temp_y,proba_temp;
...
*/	//*********************
	...
	for(i=0;i<test_vec_num;i++)
		if( testCount1[i]<=able_correct && testCount2[i]!=0 )
		{
//			printf("\n\nNo.%d test vector factorization has failed!!, testCount1=%d, testCount2=%d, degreeOfPolynomail=%d\n", i, testCount1[i], testCount2[i], degree_test[i]);
...
//			printf("\n\nNo.%d test vector: CodeWordScore=%d, testCount2=%d, degreeOfPolynomail=%d\n", i, n-testCount1[i], testCount2[i], degree_test[i]);
...
*/
	...
	if(epcount1<=able_correct && epcount2!=0)
		printf("\nThis sequence is decoding failed， and error num is %d!\n\n", epcount1);
