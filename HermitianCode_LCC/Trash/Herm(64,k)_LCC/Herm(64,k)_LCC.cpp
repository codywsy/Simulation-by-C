...
//tgorder()
#define tg_size 100	//more than k, tg_size represent the probably used pole basis num
//mono_table()
#define weight_Zsize 4
#define weight_XYsize 100	//equals to the tg_size
#define mono_ordinSize 250	//choose the value with debugging
#define monoTable_Zsize	(lm+1) 	//equals to the interpoly_Zsize
#define monoTable_Ysize 30	//large than the interpoly_Ysize
#define monoTable_Xsize (w+1)	//large than the interpoly_Xsize
#define monoTable_totalSize 100 //>index of term[max(deg_y)][w] in the pole basis + 1, less than tg_size
//polebasis()
...
//rcs()
#define rcspoly_Ysize 66	//degY+1, degY = interpoly_Ysize + max(j_1) + w*( (max(i_1)+w)/(w+1) )
#define rcspoly_Xsize 9	//degX+1, degX = max(i_1) + w
#define faiMax_Ysize 6	//change with diff w, the max degY of probably used polebasis
#define faiMax_Xsize 4	//change with diff w, the max degX of probably used polebasis
...
void main()
{
	...
	FILE *fp;
	if((fp=fopen("LCC_Herm(64,29)eta=6_normal.txt","a"))==NULL)
	{
...
*/
		...
		fp=fopen("LCC_Herm(64,29)eta=6_normal.txt","a");
