...
{	//hard decision
	int i,j;
	float d[4],temp;
	int point[4][2]={ {0,0} , {1,0} , {1,1} , {0,1} };

	for(i=0;i<(n*p)/2;i++)	
	{
		d[0]=pow( (rx_symbol[i][0]-1),2 ) + pow( (rx_symbol[i][1]-1),2 );   //(1,1)-->(0,0)
		d[1]=pow( (rx_symbol[i][0]+1),2 ) + pow( (rx_symbol[i][1]-1),2 ); 	//(-1,1)-->(1,0)
		d[2]=pow( (rx_symbol[i][0]+1),2 ) + pow( (rx_symbol[i][1]+1),2 );   //(-1,-1)-->(1,1)
		d[3]=pow( (rx_symbol[i][0]-1),2 ) + pow( (rx_symbol[i][1]+1),2 );   //(1,-1)-->(0,1)
...
void interpolation(void)
{
	int i,j,h,temp,flag,poly_lod_temp;
    ...
    for(tk=0;tk<n;tk++)
    {
		...
		for(alpha=0;alpha<m;alpha++)
			for(beta=0;beta<(m-alpha);beta++)
			{
    			...
    			flag=1;
    			poly_lod_temp=poly_lod_maxsize;
    			for(h=0;h<(lm+1);h++)
    				if(delta[h]!=0)
    				{
    					if(flag==0)
	...
	if(temp!=0)
		printf("\ninterpolation is failed, error=%d",temp);
	//***********************************
