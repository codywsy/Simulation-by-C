/****************************************
RS(15,9)code   GS algorithm  m=1   modified
*****************************************/
...
#define iter_num 15
#define tm 3
#define lm 3 //Desgined factorization output list size   (should be mended)
#define poly_Ysize 4  // Ysize==lm+1
#define poly_Xsize 20 // C approximately 
#define coeff_size 20 // the size of fac coeff
#define mono_size 20  // the size of inter_polynomial
#define fa_size 10	  // the number of layers of factorization'tree 
//******************
...
void refresh_poly_temp(int array[][poly_Xsize+1]);
int fac(int);
void DFS(int);
int cal_Qy_zero(int);
...
int Q[fa_size][poly_Ysize][poly_Xsize];
...
/*
	//********debug************
	message[0]=1;	message[1]=2;	message[2]=4;	message[3]=8;	 message[4]=3;	
	message[5]=6;	message[6]=12;	message[7]=11;	message[8]=5;
	//**************************
*/			encoder();
/*	//******debug*********
	printf("\n\nmessage=\n");
...
void decoding(void)
{
    ...
    for(i=0,j=0;j<m;j++)     
    {
    	for(h=0;h<(m-j);h++)
    	{
    		if(i<(iter_num/n))
  			{
  				alpha_beta[i][0]=j;		//alhpa
   				alpha_beta[i][1]=h;		//beta
   				++i;
   			}
   		}
   	}
...
//*****		 	// work out the new poly_lod of inter_poly[i]
    ...
    for( j=0;j<poly_Ysize;j++)
		for( h=0;h<poly_Xsize;h++)
			inter_poly_minlod[j][h]=inter_poly[minlod][j][h];
	//********************************************************

/*	//**********debug(m=1****************
	int temp1=0,temp2=0,dectect=0;
	for(i=0;i<n;i++)
	{
		for(j=0;j<poly_Ysize;j++)
			for(h=0;h<poly_Xsize;h++)
			{	
				temp1=mul( power(inter_point[i][0],h) , power(inter_point[i][1],j) );
				temp1=mul( temp1 , inter_poly_minlod[j][h] );
...
//  for(int w=0;w<=lm;w++)
//	{
//	inter_poly_minlod[0][0]=6;
//	inter_poly_minlod[1][1]=7;
	for(i=0;i<n;i++)
	{
		printf("\nwhen i=%d\t",i);
		for( int c=0; c<(iter_num/n); c++)
		{
		  for(j=0;j<poly_Ysize;j++)
		   for(h=0;h<poly_Xsize;h++)
			if( (h>=alpha_beta[c][0]) && (j>=alpha_beta[c][1]) )
		   {
			 flag1= ( fac(h) / ( fac(h-alpha_beta[c][0])*fac(alpha_beta[c][0]) ) ) * ( fac(j) / ( fac(j-alpha_beta[c][1])*fac(alpha_beta[c][1]) ) );
			 flag1= flag1%2;
			 if(flag1 != 0)
			 {
				 temp1 = mul( power(inter_point[i][0],h-alpha_beta[c][0]) , power(inter_point[i][1],j-alpha_beta[c][1]) );
				 temp1 = mul( temp1 , inter_poly_minlod[j][h] );
				 temp2 = add( temp2 , temp1 );
			 }
			}
			
			printf("Q(a,b)_%d=%d\t",c,temp2);
			temp2=0;	
		}
//		printf("\n");
	}
...
/*
	...
	{
		for(h=0;h<poly_Xsize;h++)
			printf("\t%d",inter_poly_minlod[j][h]);
		printf("\n\n");
	}
	//********************
*/
...
{
	int j,h,temp;
	int poly_temp1[poly_Ysize+1][poly_Xsize+1];
	int poly_temp2[poly_Ysize+1][poly_Xsize+1];
...
void refresh_poly_temp(int array[][poly_Xsize+1])     //clean the poly_temp
{
	int h,j;

	for(j=0;j<(poly_Ysize+1);j++)
   	{
   		for(h=0;h<(poly_Xsize+1);h++)
   		{
   			array[j][h]=0;
   		}
   	}
...
{
	int v,i,h,temp,j=0;
	int rootlist[lm];		// 2*lm is the size of rootlist

	//initialize rootlist
	for(i=0;i<(lm);i++)
		rootlist[i]=-1;			//notification, rootlist=0 also is a effective codeword
...
void cal_update_poly(int v,int u,int a)     // notification   update_temp[poly_Ysize*2][poly_Xsize*2]
{
	int i,j,g,h=0;
	int flag=0;
	int Q_update[poly_Ysize][poly_Xsize];
	int temp1[poly_Ysize][poly_Xsize],temp2[poly_Ysize][poly_Xsize];
	int update_temp1[poly_Ysize*2][poly_Xsize*2],update_temp2[poly_Ysize*2][poly_Xsize*2];

	//clean Q_update[poly_Ysize][poly_Xsize], and temp1¡¢temp2
	for(j=0;j<poly_Ysize;j++)
		for(i=0;i<poly_Xsize;i++)	
	...
	{	
		// clean up_date[mono_size*2][mono_size*2]
			for(h=0;h<(poly_Ysize*2);h++)
				for(g=0;g<(poly_Xsize*2);g++)	
		...
		{		
			for(i=0;i<poly_Xsize;i++)	//  coeff *x^i * (xy+a)^j
				{
					if( Q[u][j][i]!=0 )    
					{
						for(h=0;h<poly_Ysize;h++)     
...
*/	//***********************************
	...
	//**********debug************
	int error_1=0;	  //error_1=1 means message_poly being worked out after fac
	int error_2=0;    //erro2 is the number of codeaword error after demodulation
//	if(flag2>flag3)
	for(i=0;i<n;i++)	
		if( codeword[i] != rx_codeword[i] )
			error_2++;
...
float cal_EucDis(int reEncode_codeword[],int bicodeword_temp[],int symbol_temp[][2])
{
	...
	for(i=0;i<(n*p)/2;i++)
	{
		symbol_temp[i][0]=2*bicodeword_temp[i*2+0]-1;		//coordinate x
		symbol_temp[i][1]=2*bicodeword_temp[i*2+1]-1;		//coordinate y
	}
