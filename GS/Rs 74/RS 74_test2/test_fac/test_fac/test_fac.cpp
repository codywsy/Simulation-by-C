...
void decoding(void)
{
	...
	//**********debug*************
	//y=0
	Q[0][0][0]=5;	Q[0][0][1]=6;	Q[0][0][2]=7;	Q[0][0][3]=3;	Q[0][0][4]=5;
	//y=1
	Q[0][1][0]=7;	Q[0][1][1]=5;	
	...
	*/
	...
	//********debug**************
	printf("dec_Bicodeword=\t");
	for(i=0;i<n*p;i++)
		printf("%d\t",dec_codeword[i]);
	printf("\n\n");
...
void DFS(int u)
{
	...
	*/
	...
	/*			//********debug*****************
				printf("polynomial_Q[%d]:\n",v);
				for(int c=0;c<mono_size;c++)
				{
					for(int w=0;w<mono_size;w++)
						printf("%d\t",Q[v][c][w]);
					printf("\n\n");
				}
				//*************************
...
void choose_correct_dec_codeword(void)
{
	...
	for(i=0;i<f_i;i++)
	{
		...
		printf("reEncode=\t");
		for(int c=0;c<n;c++)
			printf("%d",reEncode_codeword[c]);
		printf("\n\n");
		//*********************
			distance_temp[i]=cal_EucDis( reEncode_codeword , bicodeword_temp , symbol_temp );
		//*********debug*********
		printf("reEncode_codeword --> bicodeword:\t");
		for(int c=0;c<n;c++)
			printf("%d%d%d\t",bicodeword_temp[c*p+2],bicodeword_temp[c*p+1],bicodeword_temp[c*p]);
		printf("\n\n");
		//************************
		//*********debug*********
		printf("bicodeword--> symbol_temp:");
		for(int c=0;c<n*p;c++)
		printf("(%d,%d)\t",symbol_temp[c][0],symbol_temp[c][1]);
		printf("\n\n");
		//************************
		//*******debug*********
		printf("distance[%d]:\t%f\n\n",i,distance_temp[i]);
...
float cal_EucDis(int reEncode_codeword[],int bicodeword_temp[],int symbol_temp[][2])
{
	...
	for(i=0;i<n*p;i++)
	{	//caution: here must use compulsory convert
