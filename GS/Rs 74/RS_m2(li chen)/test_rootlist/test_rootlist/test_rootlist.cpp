/****************************************
RS(7,4)code   GS algorithm  m=2   modified
*****************************************/
...
#define mono_size 28  // the size of inter_polynomial
...
int SNR;
double BER, FER;
int mularray[]={1,2,4,3,6,7,5};  //this array is used to the infinite field element mul
int a[7]={1,2,4,3,6,7,5};      //caution: this array must change with different sequence of infinite element

int root[]={0,1,2,3,4,5,6,7};	//n+1		be used to the factorization
...
/*	
	//***debug******
	codeword[0]=7;	codeword[1]=3;	codeword[2]=0;	codeword[3]=6;
	codeword[4]=1;	codeword[5]=4;	codeword[6]=5;
	
	printf("\nen_codeword=\t");
	for(i=0;i<n;i++)
		printf("%d\t",codeword[i]);

	rx_codeword[0]=7;	rx_codeword[1]=3;	rx_codeword[2]=0;	rx_codeword[3]=6;
	rx_codeword[4]=4;	rx_codeword[5]=4;	rx_codeword[6]=5;
...
{
	int v,i,h,temp,j=0;
	int rootlist[2*lm];		// 2*lm is the size of rootlist
...
*/
	...
	if(flag2>flag3)
		for(i=0;i<k;i++)
				if(f[min_distance_seqnum][i]!=message[i])
					error_1++;
...
//	{
		...
		printf("\nmin_distance_seqnum=%d",min_distance_seqnum);
		...
		for(i=0;i<f_i;i++)
		{
			...
			for(j=0;j<k;j++)
				printf("%d\t",f[i][j]);
			printf("\t\tdistance_temp[%d]=%f",i,distance_temp[i]);
			printf("\n");
		}
