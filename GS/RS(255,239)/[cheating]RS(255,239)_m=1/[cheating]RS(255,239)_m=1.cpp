/*******************************
(255,239) m=1 simulation programme
...
#define iter_num 255  // C
#define tm 8
#define lm 1 //Desgined factorization output list size   (should be mended)
#define poly_Ysize 2  // Ysize==lm+1
#define poly_Xsize 260 // C+1 approximately 
#define y_size 55	// mono_table size
#define x_size 1500	// mono_table size
#define poly_lod_maxsize 10000 //when k is very large, then maxsize should be modified
#define coeff_size 500 // the size of fac coeff
#define fa_size 500	  // the number of layers of factorization'tree, **depend on the size of n**
//********************************
...
float SNR;
double BER, FER;
int mularray[]={1,2,4,8,16,32,64,128,29,58,116,232,205,135,19,38,76,152,45,90,180,117,234,201,
				...
				224,221,167,83,166,81,162,89,178,121,242,249,239,195,155,43,86,172,69,138,9,18,36,
				72,144,61,122,244,245,247,243,251,235,203,139,11,22,44,88,176,125,250,233,207,131,
				27,54,108,216,173,71,142}; 
				//this array is used to the infinite field element mul

int a[]={1,2,4,8,16,32,64,128,29,58,116,232,205,135,19,38,76,152,45,90,180,117,234,201,
				143,3,6,12,24,48,96,192,157,39,78,156,37,74,148,53,106,212,181,119,238,193,159,
				35,70,140,5,10,20,40,80,160,93,186,105,210,185,111,222,161,95,190,97,194,153,47,
				94,188,101,202,137,15,30,60,120,240,253,231,211,187,107,214,177,127,254,225,223,
				163,91,182,113,226,217,175,67,134,17,34,68,136,13,26,52,104,208,189,103,206,129,
...
void main()
{
	...
	FILE *fp;
	if((fp=fopen("data_(255,239)m=1.txt","a"))==NULL)
	{
...
/*			for(i=0;i<k*p;i++)
			...
			for(i=0;i<k;i++)
				message[i]=bi_message[p*i]+2*bi_message[p*i+1]+4*bi_message[p*i+2]+8*bi_message[p*i+3]
					       +16*bi_message[p*i+4]+32*bi_message[p*i+5]+64*bi_message[p*i+6]
						   +128*bi_message[p*i+7];
...
//		printf(" Progress=%0.1f,dectect=%d\r",progress,dectec);
		fp=fopen("data_(255,239)m=1.txt","a");
...
{	//hard decision
	...
	for(i=0;i<n;i++)
		rx_codeword[i] = rx_bicodeword[p*i]+2*rx_bicodeword[p*i+1]+4*rx_bicodeword[p*i+2]+8*rx_bicodeword[p*i+3]
					 	+16*rx_bicodeword[p*i+4]+32*rx_bicodeword[p*i+5]+64*rx_bicodeword[p*i+6]
						+128*rx_bicodeword[p*i+7];
...
void interpolation(void)
{
	...
	int minlod_seq=-1,minlod=0;	//minlod_seq is the j' with delta[j']!=0
...
int add(int fac1,int fac2)
{
	...
	f=e[0]*128+e[1]*64+e[2]*32+e[3]*16+e[4]*8+e[5]*4+e[6]*2+e[7]*1; //conver to decimal
...
void DFS(int u)
{
	...
	else if(deg[u] < (k-1))		// the "D" of thesis represent k-1
	{
		...
		j=0;
		for(h=0;(h<q)&&(j<lm);h++)		// q is the size of finite field
		{
...
{
	int j,temp;
	int flag=0; //flag=0 never get in , flag=1 get in at least once
