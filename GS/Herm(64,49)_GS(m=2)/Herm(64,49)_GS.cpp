...
#define myWay
#define OpenFile fp=fopen("Herm(64,4)_GS_m=1.txt","a")
#define FrameError 229

//#define checkInter
#define checkFac	//when cheatingFac starts up, the checkFac must start up too.
//#define cheatingDecoding
//#define cheatingFac
#define cheatingChoose
...
#define interpoly_Ysize 156	//maxdeg of y is (w-1) + w*(iterNum_max/(w+1)), so the Ysize should add 1 more.
...
#define facpoly_Ysize 156	// same as the interpoly_Ysize
#define facpoly_Xsize (w+1)	// same as the interpoly_Xsize
//rcs()
#define rcspoly_Ysize 172	//degY+1, degY = interpoly_Ysize + max(j_1) + w*( (max(i_1)+w)/(w+1) ), rcspoly_Ysize = (expoly_Ysize-1) + (facpoly_Ysize-1) + 1
...
#ifdef cheatingChoose
void cheatChoose(void);
#endif
...
void demodulation(void)	//生成可信度矩阵的demodulation
{
	int i, j, u, v;
...
void interpolation()
{
	int i, j, u, v, z, temp, temp1, temp2, index_temp, lod_temp[init_polyNum];
	int lod[init_polyNum], delta[init_polyNum], J[init_polyNum], act[init_polyNum];
	int lod_min, j_min, alpha, beta, xi;
	int interpoint[n][2];	//(pointIndex,ri,mij) 
	...
	for (i = 0; i < n; i++)
	{
		interpoint[i][0] = i;	//pointIndex
		interpoint[i][1] = rxcodeword[i];	//r_i
	}
	...
	for (i = 0; i<n; i++)
	{
		for (alpha = 0; alpha<multiplicity; alpha++)
			for (beta = 0; beta<(multiplicity - alpha); beta++)
			{
	...
	for (u = 0; u<(lm + 1) && flag_iterNum; u++)
		for (j = 0; j<monoTable_Ysize && flag_iterNum; j++)
			for (i = 0; i<monoTable_Xsize && flag_iterNum; i++)
				if (mono_order[u][j][i] == lod_temp[index_temp])
				{
	...
	codewordScore = 0;
	for (i = 0; i < n; i++)
		if (rxcodeword[i] == codeword[i])
			codewordScore++;

	codewordScore *= multiplicity;
...
#ifdef checkFac
	//This is the checkFac condition in KV algorithm
...
int cal_delta(int point_temp[2], int j, int alpha, int beta)
{
