...
int Q_uncom_elem[test_vec_num_max][init_polyNum][interpoly_Zsize][interpoly_Ysize][interpoly_Xsize];
	...
	unsigned long int commonNum, uncommonNum, facNum, chooseNum;
#endif

#ifdef CoefficientDelta
...
#ifdef	_PrintComplexity_
	double commonNum_count, uncommonNum_count, facNum_count, chooseNum_count;
#endif
...
//			message[0]=1;	message[1]=2;	message[2]=1;	message[3]=3;
			...
			outputlist_count = outputlist_count + (outputlistNum - outputlist_count) / (double)(j);
			...
			if (j % 20 == 0)
			  printf("Progress=%0.1f, seqNow=%d, SNR=%2.2f, BitErrors=%2.1d, BER=%E, FrameErrors=%2.1d, FER=%E, addNum=%0.2f, mulNum=%0.2f, total_num=%0.2f, outList_num=%0.2f, Choice Errors=%2.1d, CWR=%E\r", progress, seq_num_Now, SNR, error, BER, ferror, FER, addNum_count, mulNum_count, totalNum_count, outputlist_count, ChosenWrong_SeqNum, CWR);
		...
		printf("Progress=%0.1f, seqNow=%d, SNR=%2.2f, BitErrors=%2.1d, BER=%E, FrameErrors=%2.1d, FER=%E, addNum=%0.2f, mulNum=%0.2f, total_num=%0.2f, outList_num=%0.2f, Choice Errors=%2.1d, CWR=%E\n", progress, seq_num_Now, SNR, error, BER, ferror, FER, addNum_count, mulNum_count, totalNum_count, outputlist_count, ChosenWrong_SeqNum, CWR);

		OpenFile;
		fprintf(fp, "Progress=%0.1f, seqNow=%d, SNR=%2.2f, BitErrors=%2.1d, BER=%E, FrameErrors=%2.1d, FER=%E, addNum=%0.2f, mulNum=%0.2f, total_num=%0.2f, outList_num=%0.2f, Choice Errors=%2.1d, CWR=%E\n", progress, seq_num_Now, SNR, error, BER, ferror, FER, addNum_count, mulNum_count, totalNum_count, outputlist_count, ChosenWrong_SeqNum, CWR);
		fclose(fp);

#ifdef	_PrintComplexity_
		FILE *fp= fopen("3parts Complexity.txt", "a");
		fprintf(fp, "commonNum = %0.2f\tuncommonNum = %0.2f\tfacNum = %0.2f\tchooseNum = %0.2f\n", commonNum_count, uncommonNum_count, facNum_count, chooseNum_count);
		fclose(fp);
#endif
		//****debug******
				...
				for(v=0;v<interpoly_Ysize;v++)	//max(deg_y)+1
					for(z=0;z<interpoly_Xsize;z++)	//w+1
						if (Q_uncom_elem[i][j][u][v][z] != 0)
						{
							...
							if (mono_order[u][v][z] == -1)
								printf("\n size of mono_order is not enough!!");
...
void com_elem_interpolation(int g[][interpoly_Zsize][interpoly_Ysize][interpoly_Xsize],int interpoint[][3])
{
	...
	for(i=0;i<n-eta;i++)	//wrt each point
	{
		...
		for(j=0;j<init_polyNum;j++)	//wrt each polynomial
		{
			...
			{
#ifndef CoefficientDelta
				//Hasse derivative mapping use Q(x_i,y_i,z_i)
...
#else
				delta[j] = cal_delta(interpoint[i], j, 0, 0);
#endif
		...
		for (u = 0; u<interpoly_Ysize; u++)
			for (v = 0; v<interpoly_Xsize; v++)
				if ((Q_interpoly[j][i][u][v] != 0) && (i >= beta))	//i is the degree of z
				{
					...
					int point_index=-1;
					for (h = 0; h < n; h++)
						if (point[h][0] == point_temp[0] && point[h][1] == point_temp[1])
							point_index = h;

					if (point_index == -1)
						printf("\ncal_delta_part has error\n");

					if (coeff_table[point_index][alpha][a] != 0)
					{
						b = i;
						...
						if ((flag % 2) != 0)
						{
							...
							delta_temp2 = mul(Q_interpoly[j][i][u][v], coeff_table[point_index][alpha][a]);
